...
public:
    Processor(std::vector<std::string> args, BMP& bmp) {
        try{
            Filter *crop = new Crop();
            filter_["-crop"] = crop;
            arg_count_["-crop"] = 2;
            Filter *grayscale = new Grayscale();
            filter_["-gs"] = grayscale;
            arg_count_["-gs"] = 0;
            Filter *neg = new Negative();
            filter_["-neg"] = neg;
            arg_count_["-neg"] = 0;
            Filter *sharp = new Sharp();
            filter_["-sharp"] = sharp;
            arg_count_["-sharp"] = 0;
            Filter *edge = new EdgeDetection();
            filter_["-edge"] = edge;
            arg_count_["-edge"] = 1;
            Filter *blur = new FastGaussianBlur();
            filter_["-blur"] = blur;
            arg_count_["-blur"] = 1;
